...
int main(int argc, char **argv) {
	...
	al_clear_to_color(al_map_rgb(1, 0, 1));

	al_set_target_bitmap(al_get_backbuffer(display));

	font = al_load_ttf_font("DejaVuSans-Bold.ttf", 20, NULL);
	...
	while (1)
	{
		...
		if (ev.type == ALLEGRO_EVENT_TIMER) {
			...
			bouncer_y = 250 + (10 * (16 * cos(t) - 8 * cos(2 * t) - 2 * cos(3 * t) - cos(4 * t)))*-1;
		...
		if (redraw && al_is_event_queue_empty(event_queue)) {
			...
			al_clear_to_color(al_map_rgb(150, t, 200));
			al_set_target_bitmap(al_get_backbuffer(display));
			al_draw_bitmap(bouncer, bouncer_x, bouncer_y, 0);
			al_draw_text(font, al_map_rgb(0, 0, 0), 400, 300, ALLEGRO_ALIGN_CENTER, "ERROR 'love'could not be found:( you are forever alone.exe");
			al_flip_display();
		}
	}
